...
function someTranslation() {
  ...
  console.log(translate(`Accept`));
  console.log(
    translate("Error code: %(code)s\n\n%(message)s", {
      code: "404",
...
const testLocale = () => {
  ...
  useLang("it");
  const state_1 =
    new Date().getSeconds() > 30 ? "prop.st-idle" : "prop.st-printing";
  console.log(
    translate(state_1)
  ); /* Don't not use this! Locale loader will not
  ...
  const state_2 =
    new Date().getSeconds() > 30 ? "prop.temp-amb" : "prop.temp-bed";
  ...
  const state_3 =
    new Date().getSeconds() > 30 ? translate("Pause") : translate("Resume");
  console.log(state_3);
  ...
  console.log(
    translate(
      "test with (bracket) and \"double quotes\" and 'single quotes' and `another quotes`"
    )
  );
  ...
  console.log(
    translate(
      "test multiple %(temperature).0f temperatures (%(temperature).1f) so %(temperature).2f should be same as %(temperature).2f.",
      { temperature: 36.525 }
    )
  );
  console.log(
    translate(
      "A64 temperature is too high. Measured: %(temperature).1f °C! Shutting down in 10 seconds...",
      { temperature: 280.22 }
    )
  );
  console.log(
  ...
  const query = "#job";
  translate(
    "There were some elements that was changed due to localization testing. Please remove lines 71-74 in locale_test.js.",
    { query }
  );
  document.querySelector(query)?.removeAttribute("hidden");
  translate("Translated upload project", { query: ".home-row .component p" });
  translate("Translated temperatures", {
    query: ".home-row .component:last-child p",
  });
};
