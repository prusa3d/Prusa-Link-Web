    ...
    if (temperatures.length > 1) {
        ...
        for (let i = 1; i < temperatures.length; i++) {
            ...
            for (let i = 0; i < temp_lines.length; i++)
                temp_lines[i] = temp_lines[i] + `L${x},${(250 - (2 * xy[i + 1] / (MAX_TEMP / 100)))}`;
        }
    }
...
export function updateTemperatures(temps, data) {
    ...
    const now = new Date().getTime();
    while (temps.length > 1 && temps[1][0] < now - 180 * 1000)
        temps.shift();
}
...
export function generateRandomGraph(now, minTemp, maxTemp, maxStep) {
    ...
    for (let i = 180 * 1000; i > 0; i -= process.env.UPDATE_INTERVAL) {
        graph.push([
            now - i,
            generateRandomTemp(graph.length === 0 ? undefined : graph[graph.length - 1][1], minTemp, maxTemp, maxStep)
        ]);
    }
