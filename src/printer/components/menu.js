...
export const initMenu = () => {
  ...
  langSelector.innerHTML = getLanguages().map(lang =>
    `<option value="${lang}">${lang.toUpperCase()}</option>`).join("\n");
  langSelector.value = getLanguage();

  document.getElementById("lang").addEventListener("change", (e) => {
    if (setLanguage(e.currentTarget.value))
  ...
  document.querySelectorAll("#navbar a[href]").forEach((e) => {
    ...
    if (path.length > 1) {
      switch (path[path.length - 1]) {
        case 'dashboard': translate("home.link", { ref: e }); break;
        case 'projects': translate("proj.link", { ref: e }); break;
        case 'temperature': translate("temps.title", { ref: e }); break;
      }
    }
  })
};