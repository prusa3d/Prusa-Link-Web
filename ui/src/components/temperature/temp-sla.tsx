...
const Temperature: preact.FunctionalComponent<P> = props => {
  ...
  if (temperatures.length > 1) {
    ...
    for (let i = 1; i < xy.length; i++) {
      temp_lines.push(`M${x},${250 - 2 * xy[i]}`);
    }
    for (let i = 1; i < temperatures.length; i++) {
      xy = temperatures[i];
      x = 500 - (2.66 * (now - xy[0])) / 1000;
      for (let i = 0; i < temp_lines.length; i++) {
        temp_lines[i] = temp_lines[i] + `L${x},${250 - 2 * xy[i + 1]}`;
      }
    }
  }
  ...
  return ready ? (
    <div class="box has-background-black is-paddingless">
      {!props.bigSize && (
        <p
          class="prusa-line prusa-item-title prusa-text-grey is-marginless"
          style={{ padding: 0 }}
        >
      ...
      >
        <g>
          ...
          ></rect>
          <path
            d="M 360, 285 m -6, 0 a 6, 6 0 1,0 12,0 a 6, 6 0 1,0 -12,0"
            role="presentation"
            shapeRendering="auto"
