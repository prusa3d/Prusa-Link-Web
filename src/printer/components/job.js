...
function updateJob(context, isFilePreview) {
  if (!isFilePreview) {
    const jobFile = context.job?.file;
    if (!jobFile) {
    ...
    let loading = context.state === LinkState.BUSY;
...
function updateComponent(context, isFilePreview) {
  ...
  if (dataSource.file) {
    ...
    if (dataSource.thumbnail?.ready /* && !isLoading(state) */) {
      hideLoading();
    }
  }
}
...
function setupCancelButton(state, jobId) {
  ...
  const enabled = !pendingCommand && JobPendingStates.includes(state) && state !== LinkState.BUSY;
